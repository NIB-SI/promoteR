...
# Disable progress output during knitting
if (!interactive()) {
...
library(txdbmaker) # makeTxDbFromGFF migration
...
# when more scaffolds in FASTA than in GFF
faidx = read.table(
  paste0(file.path(in.dir.1, my.file.1), ".fai"),
  sep = "\t", header = FALSE
)

# chrominfo for txdbmaker
...
my.gff3 = txdbmaker::makeTxDbFromGFF(file.path(in.dir.1, my.file.2),
                              format="gff3",
                              dataSource=NA,
...
GenomeInfoDb::genome(my.gff3) = "UniTato" # adjust as needed

str(my.gff3)
# my.gff3$user_seqlevels

# optional - check CDS with missing phase
# tmp = rtracklayer::import(file.path(in.dir.1, my.file.2))
# subset(tmp, type == "CDS" & is.na(phase))
...
# check = cbind(names(txCoordsByGene.GRangesList), names(cdsCoordsByGene.GRangesList))
...
with_progress({
  ...
  #   seq_len(n),
  #   function(i) {
  #     # ensure workers load required packages
  #     library(stats)
  #     process_element(i, cdsCoordsByGene.GRangesList, p)
  #   },
  #   BPPARAM = param
  # )
  # BiocParallel passes everything in the function environment
  # future only passes globals unless you explicitly give them as arguments

  # Win
  myVec = future_lapply(
    seq_len(n),
    function(i, p_local) {
      process_element(i, cdsCoordsByGene.GRangesList, p_local)
    },
    p_local = p
  )
})
...
```{r, upstream_downstream}




dna = readDNAStringSet(file.path(in.dir.1, my.file.1))
width(dna)
# names(dna)
